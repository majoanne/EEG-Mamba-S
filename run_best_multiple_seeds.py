...
def main():
    ...
    for seed in seeds:
        ...
        if os.path.exists(args.foundation_dir):
            state = torch.load(args.foundation_dir, map_location='cpu')
    ...
    print("\n" + "="*100)
    print(f"All seeds finished. ROC-AUC values: {[round(x, 4) for x in auc_list]}")
